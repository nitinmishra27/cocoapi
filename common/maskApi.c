 ...
 **************************************************************************/
#include "maskApi.h"#include < math.h > #include < stdlib.h >
...
void rleEncode(RLE * R,
  const byte * M, siz h, siz w, siz n) {
  siz i, j, k, a = w * h;
  uint c, * cnts;
  byte p;
  ...
  for (g = 0; g < n; g++)
    for (d = 0; d < m; d++)
      if (o[g * m + d] > 0) {
        ...
        if (i == 0) u = 1;
        else if (crowd) rleArea(dt + d, 1, & u);
...
void rleFrBbox(RLE * R,
  const BB bb, siz h, siz w, siz n) {
  siz i;
  for (i = 0; i < n; i++) {
...
int uintCompare(const void * a,
  const void * b) {
  uint c = * ((uint * ) a), d = * ((uint * ) b);
  return c > d ? 1 : c < d ? -1 : 0;
}

void rleFrPoly(RLE * R,
  const double * xy, siz k, siz h, siz w) {
